...
{
    // ViewModel dla pojedynczego elementu na liście wyników wyszukiwania
    public class SearchResultItemViewModel : ViewModelBase
    {
        ...
        public SearchResultItemViewModel(Game game)
        {
            ...
            try
            {
                ...
                using (var stream = await response.Content.ReadAsStreamAsync())
                {
                    ...
                    await Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        ...
                        Debug.WriteLine($"[SearchResultItemViewModel] Wczytano miniaturkę dla gry: {GameData.Title}"); // Komunikat debug
                    });
                }
            }
            catch (HttpRequestException e)
            {
                Debug.WriteLine($"[SearchResultItemViewModel] Błąd HTTP podczas ładowania miniaturki ({url}): {e.Message}");
                // TODO: Obsługa błędów (np. ustawienie domyślnej ikonki "brak miniaturki")
            }
            catch (Exception e)
            {
                Debug.WriteLine($"[SearchResultItemViewModel] Błąd podczas ładowania miniaturki ({url}): {e.Message}");
                // TODO: Obsługa innych błędów
            }
        }

        // Można dodać inne właściwości pochodne od GameData lub specyficzne dla wyświetlania
        // np. Title, Genre, Platform - dla łatwiejszego bindowania w XAML
